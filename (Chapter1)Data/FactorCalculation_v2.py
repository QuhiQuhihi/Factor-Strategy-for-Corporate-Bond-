...
class FactorInvesting:
    ...
    def applyCal(inputDataset, outputDataset, targetName):
        ...
        def calculate_12_month_sales_change(group, targetName: str):
            ...
            group[f'{targetName}_%change'] = (group[targetName] - group[targetName].shift(12, freq='M')) / group[targetName].shift(12, freq='M')
            return group
    ...
    def calculate_factors(self, input: pd.DataFrame) -> pd.DataFrame: 
        ...
        results = FactorInvesting.applyCal(input, self.output, 'npm')
        results = FactorInvesting.applyCal(input, results, 'debt_assets')
        results = FactorInvesting.applyCal(input, results, 'opmbd')
        results = FactorInvesting.applyCal(input, results, 'de_ratio')
        results = FactorInvesting.applyCal(input, results, 'debt_capital')      
        results = FactorInvesting.applyCal(input, results, 'debt_invcap')
...
if __name__ == "__main__":
    db_file = 'TRACE.db'
    query = "SELECT * FROM wrds_fin_ratio"
    ...
    bond_factors = pd.read_csv('bond_factors.csv')
